  ...
  if (!validationResult.isValid) {
    ...
    if (window.Toast) {
      ...
      validationResult.errors.forEach((error, index) => {
        ...
        }, 500 + (index * 800)); // Atraso sequencial para não sobrecarregar o usuário
      });
    }
  ...
  try {
    ...
    if (!success) {
      if (connectionRefused) {
        throw new Error('Não foi possível conectar ao servidor. Verifique se o backend está em execução ou tente novamente mais tarde.');
      } else if (response) {
    ...
    if (error.message.includes('Failed to fetch') || error.message.includes('conectar ao servidor')) {
      if (formErrorDisplay) {
        formErrorDisplay.textContent = 'Erro de conexão com o servidor. Verifique se o backend está em execução.';
